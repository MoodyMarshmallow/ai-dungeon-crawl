...
class Policy(Protocol):
    """Run an agent decision cycle using prior agent turns and actual feedback.

    Owns prompt construction, the full conversation, and bounded model retries.
    ...
    async def request_turn(
        self, history: Tuple[AgentTurnRecord, ...]
    ) -> AgentTurn:
...
class GameSession(Protocol):
    ...
    start/step return only when input is accepted, the process has ended, or a
    terminal game outcome is confirmed.
...
class ExecutionResult:
    """Harness/dashboard result, including partial output on failure.

    The observation is retained for the harness and dashboard; it is not
    automatically included in model feedback. Output, error, status, and truncation
    metadata form the actual feedback sent to the model.
    """
...
class GameObservation:
    ...
    `ended` means this is a terminal episode observation, including an
    authoritative death, win, or quit outcome even if the process remains live.
...
class StopExecution(Exception):
    """Abort a submitted script when execution is terminated or unavailable."""
